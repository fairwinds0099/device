...
the csv file is results of the following SQL query:
    ...
    and e.date between '06/06/2010' and '12/18/2010'
    order by e.date asc
    --all email is in 2010
    --attacks started in jun 2010
...
if __name__ == '__main__':
    ...
    # #df = pd.read_csv('/Users/apple4u/Desktop/goksel tez/data_raw/email_dtaa.csv') #1.5m records could not finish processing
    ...
    #
    # df_coded.to_csv(r'export_dataframe_all.csv', index=False, header=True) #50k records
    #

    # the baove part is disbaled to make faster predicitons.
    #if test data changes u need to re process the df_coded

    df_coded = pd.read_csv('/Users/apple4u/PycharmProjects/pythonProject/export_dataframe_all.csv')

    print('===== DECLARING GRAPH =======')
    ...
    # ATTempt for DIRECTED GRAPH
    print('===== DECLARING DIRECTED GRAPH =======')
    df_coded = df_coded.head(10000)
    DiG = nx.DiGraph()
    X = df_coded.user_common.values
    y = df_coded.coded_to.values
    ...
    network.calculate_centrality('closeness_vitality', G)
